...
public class ApplBankerHeaderTag extends TagSupport {
			...
			try
			{
				...
				if(!appno.equalsIgnoreCase(""))
				{
				 ...
				 if(applstatus.equalsIgnoreCase(""))	
					{
						strQuery = SQLParser.getSqlQuery("sel_com_banker^"+appid+"^"+appno);
						 rs=DBUtils.executeQuery(strQuery);
							if(rs.next())
							{  
							 applstatus =Helper.correctNull(rs.getString("BANKER_STATUS"));
					        }
					}
					
					
					
				  strQuery = SQLParser.getSqlQuery("sel_bankerreferencenum^"+appno);
				  rs=DBUtils.executeQuery(strQuery);
					if(rs.next())
					{  
				...
				{
					 strQuery = SQLParser.getSqlQuery("sel_com_banker^"+appid);
					 rs=DBUtils.executeQuery(strQuery);
						if(rs.next())
						{  
							appid= Helper.correctNull(rs.getString("BANKER_APPID"));
							appno=Helper.correctNull(rs.getString("BANKER_APPL_NO"));
							applname=Helper.correctNull(rs.getString("BANKER_APPLICANT_NAME"));
							applholder=Helper.correctNull(rs.getString("BANKER_APPLNHOLDER"));
				...
				String currentstatus="";
				 ...
				 int strsno=0;
					rs=DBUtils.executeLAPSQuery("maxsel_bankcerhisval^"+appno);
					if(rs.next())
					{
				...
				rs=DBUtils.executeLAPSQuery("sel_all_bankdetailscount^"+appno+"^"+strsno);
				if(rs.next())
				{
					currentstatus=Helper.correctNull(rs.getString("com_bank_sanstatus"));
				}
				...
				out.println("<td nowrap='nowrap'>");
				out.println("&nbsp;<b>Current status:</b>");
				out.println("</td>");
