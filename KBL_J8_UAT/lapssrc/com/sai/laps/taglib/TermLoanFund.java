...
public class  TermLoanFund extends TagSupport
{
	String appno,page;
	JspWriter out = null;
	...
	public int doEndTag() throws JspException
	{
		...
		try
		{
			...
			hashResult=taglib1.getTermLoanFundbased(hshValue);
			...
			if(arryRow!=null && arryRow.size()>0)
			{
				for(int i=0;i<arryRow.size();i++)
				{
					...
					String strDesc = strId+" - "+Helper.correctNull((String)arryCol.get(1)) +" - "+Helper.correctNull((String)arryCol.get(2));
		...
		{
			throw new JspException("Error in SisterConcernListTag "+e.getMessage());
		}
		return EVAL_PAGE;
	}
}