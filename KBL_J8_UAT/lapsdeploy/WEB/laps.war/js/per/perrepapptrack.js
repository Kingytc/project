...
function doRunReport()
{
	...
	window.open(path+"action/perrunapptrack.jsp?hidBeanId=repapptrack&hidBeanMethod=getData&hidMethod=getUserDetails1&txtDate="+dt+"&hidStatus="+stat+"&hidAppno="+appno+"&hidOrgCode="+code+"&hidOpFields="+outf+"&hidSelFields="+dbf+"&hidOrgName="+orgname+"&hidprodtype="+prodtype+"&hidOrgLevel="+orglevel,"track","scrollbars=Yes,toolbars=No,menubar=no ,width=750 , height=550,top=10,left=10");
	}
...
function refreshAppno()
{
	...
	count =0;
	var org="";
	var varprodtype=document.forms[0].selprodtype.value;
	if(varprodtype=="S")
	{
		...
		&& document.forms[0].cmbstatus.selectedIndex != 0)
	{
	 ...
	 var stat =document.forms[0].cmbstatus.options[document.forms[0].cmbstatus.selectedIndex].value;		
		document.all.ifrm.src=path+"action/iframeperapptrack.jsp?hidBeanMethod=getData&hidMethod=getAppno&hidBeanId=repapptrack&hidStatus="+stat+"&hidOrgCode="+org+"&prodtype="+varprodtype+"&hidOrgLevel="+orglevel;
	}
