 ...
 function doRunReport()
{
	...
	if (param !="")
	{
		window.open(path+"action/perrunappstatuswise.jsp?hidBeanId=repappstatuswise&hidBeanMethod=getData&hidMethod=getRepDetails&hidLoanType="+loantype+"&hidStatus="+stat+"&txtDate="+dt+"&txtSaveDate="+sdt+"&hidOrgCode="+code+"&hidOpFields="+outf+"&hidSelFields="+dbf+"&orglevel="+orglevel+"&hidOrgName="+orgname,"stat","scrollbars=Yes,toolbars=No,menubar=no,width=750,height=550,top=10,left=10");
	}
...
function save()
{
	...
	var beanid ="repData";
	var action="Insert";
	var source="/action/perrepappstatuswise.jsp";
...
function setParam()
{
	...
	if(document.forms[0].sellevel.selectedIndex != 0 
		...
		&& document.forms[0].selloantype.value != 0
		) 
	{
	var dt =document.forms[0].txtDate.value;
   //if (document.forms[0].txtDate.value == "")
...
//}
   ...
   count =0;
	...
	else if(document.forms[0].selloantype.value == 0)
	{
...
function doReset()
{
	...
	document.forms[0].selloantype.value="";
