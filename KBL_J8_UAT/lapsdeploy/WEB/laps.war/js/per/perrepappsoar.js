...
function runrep()
{
	...
	if (param !="")
	{
		window.open(path+"action/perrunappsoar.jsp?hidBeanId=repappsoar&hidBeanMethod=getData&hidMethod=getRepDetails&hidLoanType=P&hidPrdCode=p&hidCatFields="+rpout+"&txtDate="+dt+"&txtSaveDate="+sdt+"&hidOrgCode="+code+"&hidOpFields="+outf+"&hidSelFields="+dbf+"&orglevel="+orglevel,"cred","scrollbars=Yes,toolbars=No,menubar=no ,width=750 , height=550,top=10,left=10");
	}
...
function selectProd()
{
	...
	document.all.ifrm.src=path+"action/iframecreditrep.jsp?hidBeanMethod=getData&hidMethod=getAllUsableProducts&hidBeanId=repappper&hidProducts=allprod&prodtype=P&hidLoanType=P&hidOrgCode="+strBankVal+"&prodid="+pr;
	}
	else
	{
		alert("No Rights to view");
