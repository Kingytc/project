...
showCal(appURL,'txtDate');
showCal(appURL,'txtDate2');
...
function doRunReport()
{
	...
	var status=document.forms[0].cmbstatus.value;
	...
	window.open(path+"action/perunuserwiseapp.jsp?hidBeanId=repapptrack&hidBeanMethod=getData&hidMethod=getUserClassDetails&txtDate="+dt+"&hidAppno="+appno+"&hidOrgCode="+code+"&hidOpFields="+outf+"&hidSelFields="+dbf+"&loantype="+loantype+"&orglevel="+orglevel+"&hidstatus="+status+"&hidOrgName="+orgname+"&hidappname="+appname,"track","scrollbars=Yes,toolbars=No,menubar=no ,width=750 , height=550,top=10,left=10");
	}
...
function save()
{
	...
	if (param !="")
	{
		purl =path+"action/savereport2.jsp?BeanId="+beanid+"&Action="+action;
...
function browse()
{
	...
	prop = prop + ",left="+xpos+",top="+ypos;
	var repname=document.forms[0].repname.value;
	purl =path+"action/browsereport2.jsp?BeanId= repData&userid="+user+"&method=getProfileNames&repname="+repname+"";
	window.open(purl,title,prop);
}
...
function refreshAppno()
{
	...
	{*/
	 ...
	 //var stat =document.forms[0].cmbstatus.options[document.forms[0].cmbstatus.selectedIndex].value;		
//	document.all.ifrm.src=path+"action/iframeperusertrack.jsp?hidBeanMethod=getData&hidMethod=getUserClass&hidBeanId=perrepapptrack&hidOrgCode="+org;
