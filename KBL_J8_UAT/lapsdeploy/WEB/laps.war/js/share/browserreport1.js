...
function deleteProfile()
{
    ...
    if (document.forms[0].listsel.length > 0 && document.forms[0].listsel.selectedIndex != -1)
	{
		...
		document.forms[0].hidSourceUrl.value="/action/browserreport1.jsp";
...
function runrep()
{
	...
	beanid = trim(beanid);
	//pagename = "iframefilecreate.jsp";
	
	if(pagename=="perrunapptrack.jsp");
	{
	//alert(path+"action/"+pagename+"?hidBeanId="+beanid+"&hidBeanMethod=getData&hidMethod=getParamList&userid="+userid+"&txtprofile_name="+val+"&repname="+repname);
	window.open(path+"action/"+pagename+"?hidBeanId="+beanid+"&hidBeanMethod=getData&hidMethod=getParamList1&userid="+userid+"&txtprofile_name="+val+"&repname="+repname,"Prod","scrollbars=Yes,toolbars=No,menubar=no ,width=750 , height=550,top=10,left=10");
	window.parent.close();
	}
	if(pagename=="perrunapptrack1.jsp")
	{
	
	window.open(path+"action/"+pagename+"?hidBeanId="+beanid+"&hidBeanMethod=getData&hidMethod=getParamList2&userid="+userid+"&txtprofile_name="+val+"&repname="+repname,"Prod","scrollbars=Yes,toolbars=No,menubar=no ,width=750 , height=550,top=10,left=10");
