...
function runrep()
{
	...
	if (param !="")
	{
		window.open(path+"action/comrunappsoar.jsp?hidBeanId=repappsoar&hidBeanMethod=getData&hidMethod=getRepDetails&hidLoanType=C&hidPrdCode="+pt+"&hidCatFields="+rpout+"&txtDate="+dt+"&hidOrgCode="+code+"&hidOpFields="+outf+"&txtSaveDate="+sdt+"&hidSelFields="+dbf,"cred","scrollbars=Yes,toolbars=No,menubar=no ,width=750 , height=550,top=10,left=10");
	}
...
function save()
{
	...
	var beanid ="repData";
	var action="Insert";
	var source="/action/comrepappsoar.jsp";
...
///alert(strBankVal)
	...
	var pt = document.forms[0].prodopt.options[document.forms[0].prodopt.selectedIndex].value;
	document.all.ifrm.src=path+"action/iframecreditrep.jsp?hidBeanMethod=getData&hidMethod=getProducts&hidBeanId=repappper&hidProducts=soarprod&prodtype="+pt+"&hidLoanType=C&hidOrgCode="+strBankVal+"&prodid="+pr;
	}
	else
	{
		alert("No Rights to view");
