...
{	
	...
	else if (varsrcPage == "FC" && document.forms[0].radiobutton[5].checked)
	{
		cat=6;
	}
	else
	...
	{
	ShowAlert('121','SECURITY ID');
	return false;
	}
	if(varsrcPage == "FC" && document.forms[0].radiobutton[5].checked == true && txtval == "")
	{
	...
	if(document.forms[0].hidAppType.value=="T")
		document.all.isearch.src=appURL+"action/ops_searchresults.jsp?hidBeanGetMethod=getMasterSearch&hidBeanId=mastapplicant&txtsearch="+txtval+"&radiobutton="+rdovalue+"&hid_rowcount="+hid_rowcount+"&noofrows="+noofrows;
	else
		document.all.isearch.src=appURL+"action/searchresults.jsp?hidBeanGetMethod=getMasterSearch&hidBeanId=mastapplicant&txtsearch="+txtval+"&radiobutton="+rdovalue+"&hid_rowcount="+hid_rowcount+"&noofrows="+noofrows+"&hidCategoryType="+varCategoryType+"&checkcbs=Y&pageType="+varPageType;		
}
