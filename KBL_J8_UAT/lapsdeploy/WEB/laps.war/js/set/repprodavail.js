...
function selectValues()
{
	...
	document.all.ifrm.src=path+"action/iframesetprodrep.jsp?hidBeanMethod=getData&hidMethod=getProducts&hidBeanId=prodavail&hidProducts=prod&prodtype="+prodid;
	}
}
function selectSub()
{
	...
	document.all.ifrm.src=path+"action/iframesetprodrep.jsp?hidBeanMethod=getData&hidMethod=getProducts&hidBeanId=prodavail&hidProducts=subprod&prodtype="+prodid;
...
function runrep()
{
	...
	window.open(path+"action/runprodavailrep.jsp?hidBeanId=prodavail&hidBeanMethod=getData&hidMethod=getRepDetails&hidPrdCode="+prodtype+"&hidCatFields="+rpout+"&hidOpFields="+outf+"&txtDate="+dt+"&hidSelFields="+dbf,"Prod","scrollbars=Yes,toolbars=No,menubar=no ,width=750,height=550,top=10,left=10");
	}
}
...
function setParam()
{
	...
	{
		ShowAlert("111","Product Sub Category");
...
function save()
{
	...
	var source="/action/repprodavail.jsp";
	var repname=document.forms[0].repname.value;
	
	var param = setParam();
	if (param !="")
	{
		purl =path+"action/savereport.jsp?BeanId=repData&Action=Insert";
...
function browse()
{
	...
	var ypos = (screen.height - 320) / 2;
	prop = prop + ",left="+xpos+",top="+ypos;
	document.forms[0].hidBeanId.value="prodavail";
	purl =path+"action/browsereport.jsp?BeanId= prodavail&method=getProfileNames&repname=Product Availability Report&pagename=runprodavailrep.jsp&userid="+userid;
	window.open(purl,title,prop);
}
