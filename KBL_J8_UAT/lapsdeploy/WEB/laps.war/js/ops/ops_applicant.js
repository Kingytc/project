...
function doSave()
{
	...
	if(document.forms[0].txtperapp_yrsinpresadd.value == "S")
	{
	...
	if((document.forms[0].seledu.value)=="S")
	{	
...
function callCancel()
{
	...
	if(varconfirm)
	{
		...
		document.forms[0].hidBeanId.value="mastapplicant";
		document.forms[0].action=appURL+"action/ops_applicant.jsp?applnt=P&scode=yyy";
		document.forms[0].submit();
	}
}
...
function callModify()
{
	...
	document.forms[0].hidBeanId.value="mastapplicant";
	document.forms[0].action=appURL+"controllerservlet";
	document.forms[0].hidSourceUrl.value="/action/ops_applicant.jsp?hidBeanId=mastapplicant&hidBeanGetMethod=getData";
	document.forms[0].submit();
}
...
{
	var hid_comappid1= document.forms[0].hid_comappid1.value;
	var purl = appURL+"action/perreferences.jsp?hidBeanGetMethod= getrefData&hidBeanId=refdetails&hid_comappid1="+hid_comappid1;
	var prop = 'scrollbars=no,width=800,height=400';	
	var xpos = (screen.width - 800) / 2;
	var ypos = (screen.height - 800) / 2;
	prop = prop + ",left="+xpos+",top="+ypos;
	window.open(purl,'title',prop);
