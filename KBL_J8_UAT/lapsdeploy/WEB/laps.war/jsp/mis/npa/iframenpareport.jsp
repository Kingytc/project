...
<%
	 if (vecOrgCode.size() > 0)
	 {
		...
		{
%>
			parent.frames.document.forms[0].sel_org.length =0;
...
<%		}
		...
		{
%>
			parent.frames.document.forms[0].sel_org.length ="<%=vecOrgCode.size()%>";
<%			for(int i=0;i<vecOrgCode.size();i++)
			{
				String b = (String)vecOrgCode.get(i);
				String c = (String)vecOrgName.get(i);
%>
		    parent.frames.document.forms[0].sel_org.options[<%=i%>].text="<%=c%>";
			parent.frames.document.forms[0].sel_org.options[<%=i%>].value="<%=b%>";
