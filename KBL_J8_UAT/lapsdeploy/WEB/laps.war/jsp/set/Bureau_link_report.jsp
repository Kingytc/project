...
<%	if (objValues instanceof java.util.HashMap) {
	...
	if(strCategory.equals(""))
		strCategory=Helper.correctNull((String) hshValues.get("sessionModuleType"));
	if(strCategory.equals(""))
...

{
	...
	if(val == "ReGenReport")
	{
		document.forms[0].appno.value=document.forms[0].hidBRappno.value;
...
function funDownload(varstatus,varrefno)
{
	if(document.forms[0].rad_inner_doc.checked==false)
	{
		alert("Select Applicant");
		return false;
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="outertable">
    <tr>
		<td class="page_flow">Home -&gt; Bureau Report</td>
	</tr>
    <tr>
      <td>
        <table width="70%" border="0" cellspacing="1" cellpadding="3" class="outertable">
          <tr> 
            <td width="10%" align="center" class="sub_tab_inactive"><b><a href="javascript:callApplicant()">Applicant Details</a></b></td>
					...
					<tr>
						...
						<td width="17%" valign="top">
   						...
   						<tr>
   							...
   							<td colspan="3" class="dataHeader" align="center">Customer</td>
...
<%if(arrRow!=null && arrRow.size()>0){
						...
						<tr>				
							...
							<td width="3%"><input type="radio" name="rad_inner_doc" style="border:0" 
							onClick="fnCibilReportPDF('<%=Helper.correctNull((String)arrCol.get(0))%>','<%=Helper.correctNull((String)arrCol.get(9))%>',
							'<%=Helper.correctNull((String)arrCol.get(10))%>','<%=Helper.correctNull((String)arrCol.get(11))%>','<%=i %>',
							'<%=strChkFlag %>','<%=strStatus%>','<%=Helper.correctNull((String) arrCol.get(15))%>','<%=Helper.correctNull((String) arrCol.get(12))%>')">
							</td>
						...
						<%
							//if(("Y").equalsIgnoreCase(strFetchGenFlag)){ %>
					...
					<!--	<tr>
							...
							<input type="checkbox" name="chk_cibil" checked="checked">&nbsp;CIBIL&nbsp;</input>

							<input type="checkbox" name="chk_crif" 	 checked="checked" 
...
<input type="hidden" name="hidBRappno" value="<%= Helper.correctNull((String)hshValues.get("hidBRappno"))%>">
