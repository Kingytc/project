...
<%
	 if (vecOrgCode.size() > 0)
	 {
		...
		{
%>
			parent.frames.document.forms[0].sel_branch.length =0;
					
<%		}
		else
		{
%>
			parent.frames.document.forms[0].sel_branch.length ="<%=vecOrgCode.size()+1%>";
			parent.frames.document.forms[0].sel_branch.options[0].text="--Select--";
			parent.frames.document.forms[0].sel_branch.options[0].value="0";
<%			for(int i=0;i<vecOrgCode.size();i++)
			{
				String b = (String)vecOrgCode.get(i);
				String c = (String)vecOrgName.get(i);
%>
		    parent.frames.document.forms[0].sel_branch.options[<%=i+1%>].text="<%=c%>";
			parent.frames.document.forms[0].sel_branch.options[<%=i+1%>].value="<%=b%>";
