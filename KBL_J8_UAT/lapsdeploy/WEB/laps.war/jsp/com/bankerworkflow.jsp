...
if(hshValues!=null){
	...
	String appno = Helper.correctNull((String) hshValues.get("BANKER_APPL_NO"));
	if (appno.equalsIgnoreCase("")) {
		appno = Helper.correctNull((String) request.getParameter("hidapplno"));
	}
	String Appstatus = Helper.correctNull((String) session.getAttribute("strappstatus"));
%>
<html>
<head>
<title>Banker's Certificate - Work Flow</title>
<script>
...
function callOtherTab(val)
{
	if(val=="action")
	{
		...
		document.forms[0].hidBeanGetMethod.value="getbankerworkflow";  //changed as getAction
		document.forms[0].action=appURL+"action/bankerworkflowaction.jsp";
...
function callBanker()
{
		...
		document.forms[0].action = appURL+"action/com_bankers.jsp?hidid="+varsolid;
		document.forms[0].submit();
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
	class="outertable">
	<tr>
		<td class="page_flow">Home -> Banker's Certificate WorkFlow -> History</td>
	</tr>
</table>
...
<input type="hidden" name="hidapplno" value="<%=appno%>">
