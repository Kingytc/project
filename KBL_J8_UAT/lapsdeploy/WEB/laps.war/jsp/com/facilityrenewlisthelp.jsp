...
String strFacilityAgr=Helper.correctNull((String) hshValues.get("strFacilityAgr"));
session.setAttribute("strFacilityAgr", strFacilityAgr);
String ProposalType = Helper.correctNull((String) hshValues.get("app_proptype"));
...
if (hshValues.get("hshAppData") != null) 
{
	...
	valuesin = Helper.correctNull((String) hshRecord.get("valuesin"));
	ProposalType = Helper.correctNull((String) hshRecord.get("app_proptype"));
	...
	btnenable = ("" + Helper.correctNull(
			...
			.equals(Helper.correctNull((String) session.getAttribute("strUserId")))) ? "Y" : "N";
}
...
var checkbox_len="";
var checkbox_length="";
var varFacType="<%=Helper.correctNull((String)request.getParameter("hidrenewFac"))%>"
		...
		if(vecData.size()>1)
		{
			...
			{%>
				if(document.forms[0].chkFac["<%=l%>"].checked==true)
				{
					...
					if(vecRow!=null)
					{%>
						facsno=facsno+"<%=Helper.correctNull((String)vecRow.get(10))%>"+"@"+"<%=Helper.correctNull((String)vecRow.get(11))%>"+"@"+"<%=Helper.correctNull((String)vecRow.get(15))%>"+"@"+"<%=Helper.correctNull((String) vecRow.get(13))%>"+"@"+"<%=Helper.correctNull((String) vecRow.get(19))%>"+"@"+"<%=Helper.correctNull((String) vecRow.get(20))%>"+"~";
					<%}%>
				}
			<%}
		}
...
function callRenew()
{
	...
	{
		callCheckedFacility();
		document.forms[0].hidComappid.value=varappid;
		document.forms[0].hidType.value="R";
		...
		if(varFacType=="S")
		{
			document.forms[0].hidType.value="S";
			window.opener.document.forms[0].hidType1.value="S";
		}
		else if(varFacType=="P")
		{
			document.forms[0].hidType.value="P";
			window.opener.document.forms[0].hidType1.value="P";
			window.opener.document.forms[0].action=appURL+"action/postsancparameters.jsp?strInward=Y&pageFrom=postsanc";
		}
		else
		{
			document.forms[0].hidType.value="R";
			window.opener.document.forms[0].hidType1.value="R";
		}
	...
	<tr>
		<td align="center">
			...
			<tr class="dataheader">
				...
				<td width="10%" align="center">CBS A/C No</td>
				<td width="10%" align="center">Process Date</td>
				<td width="15%" align="center">LimitType</td>
			    <td width="15%" align="center">Nature of Limit</td>
	...
	<%
		...
		for (int i = 1; i <= vecsize; i++) {
			...
			if (vecRow != null) 
			{
				...
				strFacdesc = Helper.correctNull((String) vecRow.get(6));
				strProposed = Helper.correctNull((String) vecRow.get(8));
				strSno = Helper.correctNull((String) vecRow.get(10));
				strExist = jtn.format(Double.parseDouble(Helper.correctDouble((String) vecRow.get(7))));
				strfacnature=Helper.correctNull((String) vecRow.get(9));
				Applicationno=Helper.correctNull((String)vecRow.get(11));
				strGroup=Helper.correctNull((String)vecRow.get(12));
				strCategory=Helper.correctNull((String)vecRow.get(13));
				...
				CBS_Accno=Helper.correctNull((String)vecRow.get(15));;
				Process_date=Helper.correctNull((String)vecRow.get(14));;
			}
	%>
	<% if (strGroup.trim().equals("0") && (strCategory.equalsIgnoreCase("R")||strCategory.equalsIgnoreCase("O"))) 
	{
				j = 0;
	%>
		<tr class="datagrid">
...
<input type="hidden" name="var_Com_Newid" value="<%=Helper.correctNull((String)request.getParameter("com_newid"))%>">
<input type="hidden" name="comapp_newid" value="<%=Helper.correctNull((String)request.getParameter("com_newid"))%>">
<input type="hidden" name="hidappid" value="<%=Helper.correctNull((String)request.getParameter("hidapplicantoldid"))%>">
<input type="hidden" name="hidAppno" value="<%=Helper.correctNull((String)request.getParameter("appno"))%>">
<input type="hidden" name="hidComappid" value="">  
