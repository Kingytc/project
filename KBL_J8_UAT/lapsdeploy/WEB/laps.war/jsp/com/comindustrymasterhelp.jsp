...
function callSearch()
	{
		...
		document.forms[0].hidBeanGetMethod.value="getBSRCode";
		document.forms[0].hidBeanId.value="commappmaster";
		document.forms[0].action=appURL+"action/comindustrymasterhelp.jsp";	
...
function setValues(value1,value2,value3,value4,value5)
	{
		...
		else if("<%=strPage%>"=="systerconcern")
		{
			window.opener.document.forms[0].comapp_indcode.value=value1;
			window.opener.document.forms[0].comapp_companyid.value=value2;
		}
		else
		{		
			var value6=value1+"-"+value2;
			window.opener.document.forms[0].txt_industry_bsr_code.value = value6;
			window.opener.document.forms[0].hid_industry_code.value = value3;
		}
		window.close();
	}
function doSelect()
					...
					<tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="3" class="outertable linebor">
							<tr class="dataheader">
								...
								<td width="45%"><b>Industrial Activity</b></td>
							</tr>
						</table>
						</td>
					</tr>
							...
							<tr class="datagrid">
								<td width="15%"><a
									href="javascript:setValues('<%= Helper.correctNull((String)arrCol.get(0))%>',
                          												...
                          												'<%= Helper.correctNull((String)arrCol.get(4))%>')"><%=Helper.correctNull((String) arrCol.get(0))%></a></td>
								<td width="45%"><a
									href="javascript:setValues('<%= Helper.correctNull((String)arrCol.get(0))%>',
                          												'<%= Helper.replaceForJavaScriptString((String)arrCol.get(1))%>',
                          												'<%= Helper.correctNull((String)arrCol.get(2))%>',
...
<input type="hidden" name="hidBranchCode"
 ...
 <input type=hidden name="hidPageName" value="comindustrymasterhelp.jsp"> 
