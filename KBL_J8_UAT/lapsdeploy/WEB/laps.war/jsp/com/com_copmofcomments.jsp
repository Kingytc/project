...
<html>
<head>
<title><%if(Helper.correctNull((String)request.getParameter("PageType")).equalsIgnoreCase("DRAWDOWN")){ %>Draw Down comments<%}else{ %>COP/MOF Comments<%} %></title>
<script>
...
function doCancel()
{
	...
	document.forms[0].action=appURL+"action/com_copmofcomments.jsp";
...
function doSave()
{
	...
	document.forms[0].hidSourceUrl.value="/action/com_copmofcomments.jsp"
	document.forms[0].submit();
}
function doDelete()
{
...
<tr>
	<td colspan="3"><lapschoice:combuttonnew btnnames='Edit_Save_Cancel_Delete'	btnenable='<%=Helper.correctNull((String) request.getParameter("btnenable"))%>' /></td>
</tr>
</table>
<lapschoice:hiddentag pageid='<%=PageId%>' /> 
<input type="hidden" name="flowtype" /> 
<input type="hidden" name="yearfrom" value="1">
<input type="hidden" name="PageType" value="<%=Helper.correctNull((String)request.getParameter("PageType"))%>">
