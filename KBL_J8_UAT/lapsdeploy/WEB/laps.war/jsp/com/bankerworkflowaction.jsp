...
<%
	...
	String stratatus = Helper.correctNull((String) hshValues.get("BANKER_STATUS"));
	...
	String dbltotalrecamt=nf.format(Double.parseDouble(Helper.correctDouble((String)hshValues.get("dbltotalrecamt"))));
	String PowerAmt=nf.format(Double.parseDouble(Helper.correctDouble((String)hshValues.get("PowerAmt"))));
...
var solid="<%=Helper.correctNull((String) hshValues.get("BANKER_APPID"))%>"
var usr_class="";
var usr_dept="";
...
var varloanamount="<%=dbltotalrecamt%>";
var varsancPowerTotal="<%=PowerAmt%>";
var currentStatus="<%=Helper.correctNull((String) hshValues.get("currentStatus"))%>";
var usrclass ="<%=Helper.correctNull((String) hshValues.get("usrclass"))%>";
...
{	
	...
	if(document.forms[0].btnenable.value=="Y")
    {
		<%if (stratatus.equalsIgnoreCase("PR") || stratatus.equalsIgnoreCase("PA")) {%>
			disableButtons(false,true,true,true,true);
		<%} else {%>
	...
	else
	{
		...
		{
		<%if (stratatus.equalsIgnoreCase("PR") || stratatus.equalsIgnoreCase("PA")) {%>
		disableButtons(false,true,true,false,true);
		...
		if(varSancAuth=="Y" && Integer.parseInt(usrclass)>=85)
		{
			document.forms[0].cmdsanction.disabled=false;
			document.forms[0].cmdreject.disabled=false;
		}
		...
		<%if(Helper.correctNull((String) hshValues.get("currentStatus")).equalsIgnoreCase("EXPIRED"))
		{%>
		document.forms[0].cmdvalidityextension.disabled=false;
		<%}%>
	}
...
function disableButtons(bool1,bool2,bool3,bool4,bool5)
{
	...
	document.forms[0].cmdvalidityextension.disabled=bool5;
}
...
{	
	...
	document.forms[0].hidBeanMethod.value="updateBankerAction";
	document.forms[0].hidBeanGetMethod.value="getBankerAction";
...
function doValidityextension()
{	
//alert("VE");

if(document.forms[0].txt_valextdate.value=="")
{
	ShowAlert('111', "Validity Extension date"); 	
	document.forms[0].txt_valextdate.focus();
	return;
}

if(document.forms[0].txt_reasonext.value=="")
{
	ShowAlert('111', "Validity Extension Reason"); 	
	document.forms[0].txt_reasonext.focus();
	return;
}


	document.forms[0].hidAction.value="ValidityExtension";
	document.forms[0].hidSourceUrl.value="/action/bankerworkflow.jsp"; 
	document.forms[0].hidBeanId.value="comsolvency";
	document.forms[0].hidBeanMethod.value="updateBankerAction";
	document.forms[0].hidBeanGetMethod.value="getBankerAction";
	document.forms[0].action=appURL+"controllerservlet";
	document.forms[0].submit();
	alert("Your Validity has been Extended successfully");
...
function doSanction()
{
	...
	<%if (stratatus.equalsIgnoreCase("OP")) {%>
		if(parseFloat(varloanamount)>parseFloat(varsancPowerTotal))
		{
				 ...
				 alert("Delegated power under Banker Certificate exceeds the maximum limit  -> "+varsancPowerTotal+"  hence cannot be sanctioned");
				 return;
...
function callOtherTab(val)
{
	 if(val=="history")
	 {
	 document.forms[0].action=appURL+"action/bankerworkflow.jsp?hidBeanGetMethod=getBankerAction&hidBeanId=comsolvency";
	 document.forms[0].submit();  
	 }
}
...
<tr>
		...
		<td width="50%" align="center" class="sub_tab_active"> Banker's Certificate WorkFlow</td>
</tr>
		
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="outertable">
   	<tr>
		<td class="page_flow">Home ->  Banker's Certificate WorkFlow  -> Action</td>
	</tr>
</table>
							...
							.get("app_reviewdate"))%>"
													...
													--><tr>
														...
														<td><textarea name="txt_sanctioncomments" cols="150"
															rows="3" onKeyPress="textlimit(this,254)" onKeyUp="textlimit(this,254)"><%=Helper.correctNull((String) hshValues
							.get("BANKER_COMMENTS"))%></textarea></td>
													</tr>
													
													<%if(Helper.correctNull((String) hshValues
															.get("currentStatus")).equalsIgnoreCase("EXPIRED"))
													{%>
													<tr>
													<td>Validity Extension Due date</td>
													<td><input type="text" name="txt_valextdate"
																	size="12"
																	value="<%=Helper.correctNull((String) hshValues
							.get("VAL_EXT_DATE"))%>"
							maxlength="10"
																...
																<a alt="Select date from calender" href="#"
																	onClick="callCalender('txt_valextdate')"
																	onMouseOver="window.status='Date Picker';return true;"
																	onMouseOut="window.status='';return true;"><img
																	src="<%=ApplicationParams.getAppUrl()%>img/calen.gif"
																	border="0" alt="Select date from calender"></a></td>
																	...
																	<td><textarea name="txt_reasonext" cols="150"
															rows="3" onKeyPress="textlimit(this,254)" onKeyUp="textlimit(this,254)"><%=Helper.correctNull((String) hshValues
							.get("REASON_EXTENSION"))%></textarea></td>
															</tr>
															<%} %>
												</table>
						</td>
...
<lapschoice:combuttonnew btnnames='Sanction_Reject_Undo_Send_ValidityExtension' btnenable='<%=Helper.correctNull((String) session.getAttribute("strBtnenable"))%>'/>
